...
public class AG
{
	...
	public static int[] GA_Algorithm_FUNC1(int size_of_population, int ev) throws IOException
	{
		...
		FileWriter the_best_solutions = new FileWriter("The_Best_Solutions.txt"); // Plik najlepszymi z najlepszych rozwiazaniami.
		FileWriter average_solutions = new FileWriter("Average_Solutions.txt"); // Plik najlepszymi z najlepszych rozwiazaniami.
		float the_best_solution=0; // Najlepsze rozwiazanie.
		int[]the_best_solution_chromoson=new int[mi*n]; // Najlepszy chromoson.
		...
		while(true)
		{
			...
			float current_best_solution=0; // Maksymalna wartosc funkcji ~ wynik najlepszy.
			index_of_best_chromoson=0; // Indeks chromosonu, który ma najlepsze rozwiazanie.
			float average_solution=0; // Suma uzyskanych wartoœci funkcji.
			for(int j=0;j<size_of_population;j++)
			{
				float current_solution=AG.Calc_Func1(population[j],mi,n);
				average_solution+=current_solution;
				if(current_solution > current_best_solution)
				{
					current_best_solution=current_solution;
					index_of_best_chromoson=j;
				}
			}
			average_solution/=size_of_population; // Obliczanie sredniej wartoœci funkcji przystosowania dla chromosonu.
			//TODO: 2.1 Sprawdz czy nowy najlepszy jest najlepszy od obcnego najleszpego.
			if(the_best_solution<current_best_solution)
			{
				the_best_solution=current_best_solution;
				the_best_solution_chromoson=population[index_of_best_chromoson];
			}
			if(i%1==0) //TODO: 2.2 Dodawanie najlepszej wartosci funkcji oraz obecnej najlepszej wartosci funkcji przystosowania co iteracje.
			{
				System.out.println(current_best_solution);
				current_best_solutions.append(current_best_solution+"\n"); 
				delta_ev.append(i+"\n"); 
				the_best_solutions.append(the_best_solution+"\n");
		...
		current_best_solutions.close(); // Zamkniecie pliku: Current solutions.
		the_best_solutions.close(); // Zamkniecie pliku: The best solutions.
		average_solutions.close(); // Zamkniecie pliku: Average solutions.
		return the_best_solution_chromoson;
	}
	//END REGION-------------------------------------------------------------------------------------------------]
}
